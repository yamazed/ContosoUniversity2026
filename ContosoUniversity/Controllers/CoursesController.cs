...
namespace ContosoUniversity.Controllers
{
    public class CoursesController : BaseController
    {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (teachingMaterialImage != null && teachingMaterialImage.ContentLength > 0)
                {
                    ...
                    try
                    {
                        ...
                        {
                            var oldPath = course.TeachingMaterialImagePath.TrimStart('~', '/');
                            var oldFilePath = Path.Combine(_environment.WebRootPath, oldPath);
        ...
        public ActionResult DeleteConfirmed(int id)
        {
            ...
            if (!string.IsNullOrEmpty(course.TeachingMaterialImagePath))
            {
                var path = course.TeachingMaterialImagePath.TrimStart('~', '/');
